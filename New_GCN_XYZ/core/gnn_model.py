...
try:
    ...
    HAS_TORCH = False
    print(f"Warning: PyTorch not available ({e}). Using numpy fallback.")
...
def generate_heatmap(unique_nodes, model_path=None, temperature=1.0, train=False):
    ...
    # If PyTorch is not available, use distance-based heuristic
    if not HAS_TORCH:
        print("Using distance-based heuristic (PyTorch not available)")
...
def _generate_distance_based_heatmap(unique_nodes, temperature=1.0):
    """
    Fallback heatmap generation using distance-based heuristic.
    Assigns higher probability to shorter edges.
    """
    num_nodes = len(unique_nodes)
    coords = np.array(unique_nodes)
    ...
    # Convert distances to probabilities (shorter = higher probability)
    # Use softmax with negative distances
    ...
    for i in range(num_nodes):
        ...
        exp_scores[i] = 0  # Mask self-loop
        heatmap[i, :] = exp_scores / (np.sum(exp_scores) + 1e-9)
